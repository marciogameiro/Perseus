...
public:
    ...
    num makeFromComplex(const Complex<C,BT>& topers, bool truncate = false);
	// num makeFromComplex(const Complex<C,BT>&, bool);
	bool makeBDChains(PCMAP& corr, bool makez2 = true);
	// bool makeBDChains (PCMAP&,bool);

	// Persistence Routines
	void showBetti(ostream& out = cout); // shows betti numbers for each frame to std out
	...
	void showInts(ostream& out = cout); // shows persistence intervals for all dimensions
	...
	void REMOVE_PIVOT_ROW(const PCell<C,BT>* cell, PCCHAIN& delta, bool trace = false);
	// void REMOVE_PIVOT_ROW(const PCell<C,BT>*, PCCHAIN&, bool);
	void COMPUTE_INTERVALS(const Complex<C,BT>& other, bool makegens = false, bool truncate = false);
