...
public:
	...
	pair<num,bool> makeFromFile(ifstream& infile, bool hasbirths = true, bool mfd = true, bool hasdimcap = false); // create from file
	...
	bool makeCellComplex(const vector<Simplex<C,PS,BT>*>& topsimps, MComplex<C,BT>& simpcomp, bool inheritBirths = true);
	// bool makeCellComplex(const vector<Simplex<C,PS,BT>*>&, MComplex<C,BT>&, bool);
	bool buildTopSimpMap (const vector<Simplex<C,PS,BT>*>& topsimps, bool makeverlist = true); // populates map, and verlist optionally
	// bool buildTopSimpMap (const vector<Simplex<C,PS,BT>*>&,bool); // populates map, and verlist optionally
	bool addSimpFaces(bool inheritBirths = true); // adds faces descending from given dimension
	...
	void Destroy(bool deep = false); //cleans out the map structure, optionally deletes simplices!
	// void Destroy(bool); //cleans out the map structure, optionally deletes simplices!
	void getVerts(PT_SET&); // extracts all vertices;
	num populateMap(const vector<Simplex<C,PS,BT>*>& topsimps, bool inheritBirths = true); // builds map with faces etc.
	...
	bool makeFacesOf(Simplex<C,PS,BT>* simp, bool inheritBirths = true); // makes faces of single simplex
    ...
    bool topWrapper(const vector<Simplex<C,PS,BT>*>& topsimps, string fname = "output");
