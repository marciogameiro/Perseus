...
public:
    ...
    bool buildTopCubeMap(const num topdim, const vector<Point<PS>*>& anchs, const vector<BT>* births = NULL); // populate map using anchors
	...
	void writeComplex(MComplex<C,BT>& mycomp, bool clearself = false); // writes from map to complex
	// void writeComplex(MComplex<C,BT>&,bool); // writes from map to complex
	void writeComplexExcept(MComplex<C,BT>& mycomp, const BT& except, bool clearself = false); // writes all but cells of given birth
